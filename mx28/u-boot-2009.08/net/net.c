...
static void TestFixtureStart(void);
...
static void
TestFixtureHandler (uchar * pkt, unsigned dest, unsigned src, unsigned len)
{
	IPaddr_t tmp;
	volatile IP_t *ip = (volatile IP_t *)pkt;

	const unsigned long test_fixture_ip = (192 | (168<<8) | (28<<16) | (99<<24));

	tmp = NetReadIP((void *)&ip->ip_src);

	if (tmp != test_fixture_ip)
		return;

	printf("\x1b[37;45m!!!!!! TEST FIXTURE PRESENT !!!!!!\x1b[0m\n");
	NetState = NETLOOP_SUCCESS;
}

static void
TestFixtureTimeout (void)
{
	eth_halt();
	NetState = NETLOOP_FAIL;	/* we did not get the reply */
}
...
static void TestFixtureStart(void)
{
	...
	printf ("Using %s device\n", eth_get_name());
#endif	/* CONFIG_NET_MULTI */
	NetSetTimeout (2000UL, TestFixtureTimeout);
	NetSetHandler (TestFixtureHandler);
}
...
extern int g_watch_for_trulink_test_fixture_signal;
...
NetReceive(volatile uchar * inpkt, int len)
{
	...
	if(g_watch_for_trulink_test_fixture_signal)
	{
		// FIXME: Magic should not be in source code. Move it to an
		// external source.
		const uchar magic[] = {
			};
		const int len = sizeof(magic);

		if((NetRxPacketLen > len) && (0 == strncmp(magic, NetRxPacket + (NetRxPacketLen - len), len))) {
			g_watch_for_trulink_test_fixture_signal = 2;
			if(TestFixtureHandler == packetHandler) {
