...
{
	send_cmd(p_acc.get_sockfd(), MSG_NOSIGNAL, "stat: rssi=%d creg_mode=%d psnt_mode=%d AcT=%d network_type=%d\n\r",
		g_rssi,
		g_creg_mode,
		g_psnt_mode,
		g_AcT,
...
{
	ats_logf(ATSLOG_ERROR, "pp_up received");
	g_RedStone.ModemState(4);
	return 0;
}
static int ac_ppp_down(AdminCommandContext &p_acc, int p_argc, char *p_argv[])
{
	ats_logf(ATSLOG_ERROR, "pp_down received");
	g_RedStone.ModemState(2);
	return 0;
}
...
//	bool ER_psnt_init = false;
	bool ER_creg = false;
	const int poll_rate = config.get_int("DataPollRate");
	...
	for(;;)
	{
		...
		if (g_RedStone.ModemState() >= 2)  // only look at signal strength if we have a sim
...
//			g_RedStone.ModemState(0);
		...
		{
			g_RedStone.ModemState(1);  // set state to 'talking to internal modem'
			bModemStateSet = true;
			ats_logf(ATSLOG_ERROR, "Communication with modem module established.  Setting ModemState to 1");
		}
...
int main(int argc, char* argv[])
{
...

	{
		...
		config.set("DataPollRate", db.GetValue("Cellular", "DataPollRate", "1"));
