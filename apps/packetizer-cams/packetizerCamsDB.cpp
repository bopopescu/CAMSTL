...
PacketizerCamsDB::PacketizerCamsDB(MyData& pData, const ats::String& p_packetizerdb_name, const ats::String& p_packetizerdb_path) :
...
void PacketizerCamsDB::start()
{
	PacketizerDB::start();

	if(!createCurrTable())
	{
		ats_logf(ATSLOG_ERROR,"%s,%d: Could not create 'current_table' in database. Current-priority messages will not be sent.", __FILE__, __LINE__ );
	}

}

bool PacketizerCamsDB::createCurrTable()
{
...
bool PacketizerCamsDB::copy(const ats::String& p_table, ats::StringMap &p_sm)
{
	...
	ats_logf(ATSLOG_DEBUG, " %s, %d: dbcopy() query:\n%s", __FILE__, __LINE__, buf.c_str());
	db_monitor::DBMonitorContext db;

	if(!dbquery(db, m_packetizerdb_name, buf))
	{
		ats_logf(ATSLOG_DEBUG," %s, %d:dbcopy() Insert into packetizer_db failed", __FILE__, __LINE__ );
		return false;
	}
...
bool PacketizerCamsDB::dbcopy(int p_mid)
{
	ats::StringMap sm;
	db_monitor::DBMonitorContext db;
...
void PacketizerCamsDB::dbselectbacklog(std::vector<int>& p_mid, int p_limit)
{
	db_monitor::DBMonitorContext db;
	{
		ats::String buf;
		...
		if(!err.empty())
		{
			ats_logf(ATSLOG_ERROR, "%s,%d: Error: %s", __FILE__, __LINE__, err.c_str());
			return;
		}
...
bool PacketizerCamsDB::dbquery_curr_msg_from_packetizerdb(ats::StringMap& sm)
{
	ats::String buf;
...
//		ats_logf(ATSLOG(1), "%s: Database table size is zero ",__PRETTY_FUNCTION__);
	...
	for(size_t i = 0; i < db.Table().size(); ++i)
	{
		...
		for(it = m_DBPacketizerColumnNameType.begin(); it != m_DBPacketizerColumnNameType.end(); it++)
		{
			...
			{
				ats_logf(ATSLOG_ERROR, "%s,%d : Timestamp format wrong ", __FILE__, __LINE__);
				return false;
			}
...
bool PacketizerCamsDB::dbload_messagetypes(std::map<int,int>& p_mt)
{
	ats_logf(ATSLOG_INFO, " %s,%d: Loading message types.", __FILE__, __LINE__);
	...
	ats_logf(ATSLOG_INFO, "%s, %s: Database result returned: Table size:%d", __FILE__, __PRETTY_FUNCTION__, db.Table().size());

	for(size_t i = 0; i < db.Table().size(); i++)
	{
		const db_monitor::ResultRow& row = db.Table()[i];
		ats_logf(ATSLOG_INFO, "%s, %s: Database row %s, %s", __FILE__, __PRETTY_FUNCTION__, row[0].c_str(), row[1].c_str());
...
int PacketizerCamsDB::dbquery_SelectPriorityOneMessage()
{
	ats::String buf;
