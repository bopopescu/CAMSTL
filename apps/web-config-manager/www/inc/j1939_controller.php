...
if(!empty($_REQUEST))
{
	...
	switch($_REQUEST['op'])
	{
		case "updateSettings":
			...
			header("location:http://".$_SERVER['HTTP_HOST']."/device/j1939/index.php?success=".$result['success']."&module=add".$result['module']."Template&codes=".implode(",",$result['codes'])."&fields=".$result['fields'].$result['getParams']);
			break;
		case "addTemplate";
		...
		case "deleteTemplate":
			...
			echo "http://".$_SERVER['HTTP_HOST']."/device/j1939/index.php?success=".$result['success']."&module=".$result['module']."&codes=".implode(",",$result['codes'])."&fields=".$result['fields'].$result['getParams'];
			break;
		case "activateTemplate":
			$result = activateTemplate($dbconfig, trimRequest($_REQUEST));
			header("location:http://".$_SERVER['HTTP_HOST']."/device/j1939/index.php?success=".$result['success']."&module=".$result['module']."&codes=".implode(",",$result['codes'])."&fields=".$result['fields'].$result['getParams']);
			break;
		default:
			header("location:http://".$_SERVER['HTTP_HOST']."/device/j1939/index.php?success=".$result['success']."&module=".$result['module']."&codes=".implode(",",$result['codes'])."&fields=".$result['fields'].$result['getParams']);
	}
...
{
	header("location:http://".$_SERVER['HTTP_HOST']."/device/j1939/index.php");
}
