...
if (!empty($_REQUEST)) {
  ...
  if (!empty($_REQUEST['loginProfile']) && ($_SESSION['M2M_SESH_USERID'] == $_REQUEST['loginProfile'] || $_SESSION['M2M_SESH_USERAL']  == 1 || ($_SESSION['M2M_SESH_USERAL']  == 100 && $_REQUEST['loginProfile'] != 1 ) || ($_SESSION['M2M_SESH_USERAL']  == 200 && $_REQUEST['loginProfile'] == 3))) {
    if (!empty($_REQUEST['authPwd']) && !empty($_REQUEST['newPwd'])  && !empty($_REQUEST['confirmNewPwd'])) {
      ...
      if (strcasecmp($_REQUEST['newPwd'], $_REQUEST['confirmNewPwd']) === 0) {
        ...
        if ($passwd_ctrl->verifyUserCreds($_SESSION['M2M_SESH_USERID'], $_REQUEST['authPwd'])) {
          //change the password
          if ($passwd_ctrl->changeUserPassword($_REQUEST['loginProfile'], $_REQUEST['newPwd'])) {
