...
if(!empty($_REQUEST))
{
	...
	header("location:https://".$_SERVER['HTTP_HOST']."/device/zigbee/index.php?success=".$result['success']."&module=".$result['module']."&codes=".implode(",",$result['codes'])."&fields=".$result['fields'].$result['getParams']);
}
else
{
	header("location:https://".$_SERVER['HTTP_HOST']."/device/zigbee/index.php");
}
